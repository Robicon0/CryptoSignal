...
async function cgFetch(cgPath) {
  const cached = _cgCache.get(cgPath);
  if (cached && Date.now() - cached.ts < CG_TTL) return cached.data;
  const r = await fetch(`https://api.coingecko.com/api/v3${cgPath}`, {
    timeout: 10000,
    headers: { 'User-Agent': 'CryptoSignal/1.0', 'Accept': 'application/json' },
  });
  if (!r.ok) {
    const text = await r.text().then(t => t.slice(0, 200));
    throw new Error(`CoinGecko ${r.status}: ${text}`);
  }
  const data = await r.json();
  _cgCache.set(cgPath, { data, ts: Date.now() });
  return data;
}
...
app.get('/api/coingecko/coin/:id', async (req, res) => {
  try {
    const data = await cgFetch(
      `/coins/${encodeURIComponent(req.params.id)}` +
...
app.get('/api/watchlist-news', async (req, res) => {
  const tokens = (req.query.tokens || '').split(',').map(t => t.trim().toLowerCase()).filter(Boolean);
